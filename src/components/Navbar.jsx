    ...
    <>
      <nav className="fixed w-screen top-0 z-50">
        <div className="flex justify-end p-4 sm:p-6">
          <button onClick={toggleMenu} className="rounded-md p-2 text-white">
            {isOpen ? (
              ...
              {LINKS.map((link) => (
                <motion.li key={link.id} variants={linkVariants}>
                  <a
                    href={`#${link.id}`}
                    onClick={toggleMenu}
                    className="text-5xl font-semibold uppercase tracking tracking-wide hover:text-orange-300 lg:text-9xl"
                  >
                    {link.name}
                  </a>
                </motion.li>
